...
function listDepts() {
    ...
    // console.log(res);
    var table = new Table({
    	head: ['ID', 'Dept','Overhead'], colWidths: [5, 15, 10]
    });
    console.log("\n\n\tBAMAZON Departments");
    console.log("\t----------------------");
    for (var i = 0; i < res.length; i++) {
          ...
          table.push(
          	[res[i].department_id, res[i].department_name, "$" + res[i].over_head_costs.toFixed(2)]
          	);
...
function viewProductSales(){
    ...
    // console.log(res);
    var table = new Table({
    	head: ['ID', 'Dept', 'Overhead', 'Sales','Profit'], colWidths: [5, 15, 10, 10,10]
    });
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      table.push(
      	[res[i].department_id, res[i].department_name, "$" + res[i].over_head_costs.toFixed(2), "$" + res[i].sales.toFixed(2), "$" + (res[i].sales-res[i].over_head_costs).toFixed(2)]
      	); 
